...
1. To start of the  analysis, the two datasets of ratings and recipes were merged with a left join on the 'id' and 'recipe_id' column in order to begin the datat cleaning process.

After this first step, the merged_df had the columns and rows of:
...
1. Secondly, in the ratings column, '0' ratings were replaced with NaN because 0 is not a valid part of the rating scale and np. NaN represents the absence of a rating better. By replacing these values, we are able to maintain the integrity  the data and improve the accuracy of analysis.


1. To analyze average ratings, the column`'average_rating'` was added to merged_df.


1. The strings in columnns 'tags', 'nutrition', 'steps', 'ingredients' were into actual lists.


1. A column for the Number of Tags in Each Recipe was added as well.
...
In the Univariate Analysis, we looked at the 
...
Looking at the missingness in the 'ratings' column, we found that there could be possible correlations between the values missing and the number of steps or ingredients in the recipes.
...
A permutation test was run and the observed differences between recipes with ratings missing and ratings not missing were used in the test statistic.
...
Unfortunately there was an issue running the Permutation tests, but during our testing, these statistics were observed:
...
A permutation test was run in order to test if there was a correlation between the rating of the recipe and when the cook time is high or low.
...
**Observed Difference in Means:** 0.0254312851082954
On average, our results show that recipes with lower cooking times have ratings that are approximately 0.025 higher than recipes with higher cooking times.

**P-value:** 0.0
Our p-value of 0.0 indicates that, under the null hypothesis, the probability of observing a difference as extreme as 0.025 is essentially zero.

**Conclusion**
As the p-value is less than our significance level, this suggests that there is a statistically significant difference in average ratings between recipes with low cooking times and those with high cooking times. Therefore, we can support the alternative hypothesis that recipes with lower cooking times have higher average ratings.

This shows that, on average, recipes with lower cooking times have ratings that are approximately 0.025 higher than recipes with higher cooking times.Due to our p-value that is less than our reasonable significance level at 0.05, we reject the null hypothesis. There is statistical proof of a significant difference in average ratings between recipes with low cooking times and those with high cooking times, which supports out alternative hypothesis that recipes with lower cooking times have higher average ratings.
...
For the final model, we are including the TF-IDF values of words from the columns of 'tags', 'description', and 'review', along without initial parameters.To do this, we've createed a copy of our Merged_df and transforms values in 'tags' column in copied dataframe to string again.

## Fairness Analysis

In the Fairness Analysis of the model, we ae comparing recipes under or over 30 minutes.
